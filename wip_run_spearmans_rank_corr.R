...
# Calculate correlation matrix, rounding to two decimal places
out_mat <- round(cor(df_data_only_pos),2)

# Coerce to dataframe
out_df <- as.data.frame(as.matrix(out_mat))

# Save output NOTE that the column names are shifted left by one position.
write.table(out_df, file = 'corr_out2.tsv', row.names = TRUE,
